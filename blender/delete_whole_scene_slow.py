...
def set_lamp(world_vec, shadow_size, strength, location, rotation, index):
  lamp_data = bpy.data.lamps.new(name='Lamp%d' %(index), type='SUN')
  lamp_object = bpy.data.objects.new(name='Lamp%d' %(index),
      object_data=lamp_data)
  bpy.context.scene.objects.link(lamp_object)
  bpy.data.objects["Lamp%d" %(index)].data.type = 'SUN'
  bpy.data.objects["Lamp%d" %(index)].location = location
  bpy.data.objects["Lamp%d" %(index)].rotation_euler = rotation
  lamp = bpy.data.lamps["Lamp%d" %(index)]
...
def set_camera(world_vec, location, rotation, index):
  camera = bpy.data.cameras.new(name="Camera%d" %(index))
  camera_object = bpy.data.objects.new(name="Camera%d" %(index), object_data=camera)
  bpy.context.scene.objects.link(camera_object)
  bpy.context.scene.camera = camera_object
  bpy.data.objects["Camera%d" %(index)].location = location
  bpy.data.objects["Camera%d" %(index)].rotation_euler = rotation
  ...
  bpy.data.cameras["Camera%d" %(index)].clip_end = max(200, math.sqrt(x*x+y*y+z*z)*2)
...
def set_new_scene(prev_scene, index):
  print("first")
  ...
  print(list(bpy.data.scenes))
  print(list(bpy.data.worlds))
  bpy.data.scenes.remove(bpy.data.scenes[prev_scene])
  print("after delete")
  print(list(bpy.data.scenes))
  print(list(bpy.data.worlds))
  ...
  print_planes(world_vec, visible_planes, plane_scale, plane_disp,
      plane_material_name)
  set_camera(world_vec, camera_location, camera_rotation, index)
  set_default_camera_view()
  print_time(time_location, camera_rotation)
  return bpy.data.scenes[0].name, spheres
...
if __name__ == "__main__": 
  ...
  for i in range(start_frame, end_frame):
    prev_scene, spheres = set_new_scene(prev_scene, i)
    for j in range(species_size):
      time, c = load_coords(f)
      if len(c):
