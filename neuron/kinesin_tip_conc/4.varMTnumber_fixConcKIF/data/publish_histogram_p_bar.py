...
def plot_figure(data, row_labels, col_labels, abs_val, plot_cols, plot_rows,
  ...
  for i in range(pcols):
    y = data[bins-1:bins, plot_rows[0]-1:plot_rows[1], plot_cols[0]+i-1:plot_cols[0]+i]
    if sub_rows:
      y = np.subtract(y,
          data[bins-1:bins, sub_rows[0]-1:sub_rows[1], plot_cols[0]+i-1:plot_cols[0]+i])
  ...
  ax.set_ylabel('Cytosolic kinesin concentration at\nneurite tip (% of max)', fontsize=fontsize)
  ax.set_xlabel('Number of MT', fontsize=fontsize)
  ax.set_xticks(x)
  ...
  ax.legend(rects, legends, loc='lower left', title="MT binding\nprobability",
