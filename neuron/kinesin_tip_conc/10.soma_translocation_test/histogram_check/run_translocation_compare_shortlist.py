...
def initialize(start_time, end_time, bin_no):
  filenames = np.transpose(sorted(glob.glob("histogram_*.csv")))
  print filenames
  for i in range(len(filenames)/4):
    print "i:",i
    fig, ax = plt.subplots()
    for j in range(4):
      file = filenames[i*4+j]
      print file, i, j, i*4+j
      ...
      for k in ycols:
        tmp = data[0:drows, k:k+1].reshape(drows/bins, bins)[:, bin_no-1:bin_no]
        y = np.add(y, tmp)
      ax.plot(x, y)
  plt.show()
