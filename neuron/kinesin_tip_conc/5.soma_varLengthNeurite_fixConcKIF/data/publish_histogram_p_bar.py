...
def plot_figure(data, row_labels, col_labels, abs_val, plot_cols, plot_rows,
  ...
  ax.set_ylabel('Cytosolic kinesin concentration at neurite tip with p=0.5 (% of max)', fontsize=fontsize)
  ax.set_xlabel('% elongation of neurite #4', fontsize=fontsize)
  ax.set_xticks(x)
  ax.set_xticklabels(np.multiply(np.transpose(xticklabels)[0], 10).astype(int))
