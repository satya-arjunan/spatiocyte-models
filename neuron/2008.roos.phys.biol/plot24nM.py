...
fileNames = ["IterateLog.24nM.csv", "Fig.5.24nM.sim.csv", "Fig.5.24nM.exp.csv"]
legendTitles = []
lines = ['-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-', '-']
...
P.plot(np.divide(data[0],60.0), np.divide(combined, amax), linewidth=1.5, label=fileNames[0].split('.csv')[0])

data = np.genfromtxt(fileNames[1], delimiter=',', skip_header=0).T
P.plot(data[0], data[1], linewidth=0, marker='o', color='k', markersize=5, label=fileNames[1].split('.csv')[0])

data = np.genfromtxt(fileNames[2], delimiter=',', skip_header=0).T
P.plot(data[0], data[1], linewidth=0, marker='o', color='r', markersize=10, label=fileNames[2].split('.csv')[0])
