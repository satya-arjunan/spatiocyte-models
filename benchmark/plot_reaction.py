...
legendTitles = ['eGFRD ($r=10\ \mathrm{nm},\ \mathrm{RT}=2897\ \mathrm{s}$)','eGFRD ($r=1\ \mathrm{nm},\ \mathrm{RT}=2232\ \mathrm{s}$)','Smoldyn ($\Delta t=1\ \mathrm{ms},\ \mathrm{RT}=17\ \mathrm{s}$)','Smoldyn ($\Delta t=67\ \mathrm{\mu s},\ \mathrm{RT}=243\ \mathrm{s}$)','Spatiocyte ($\Delta t=1\ \mathrm{ms},\ r=38.73\ \mathrm{nm},\ \mathrm{RT}=11\ \mathrm{s}$)','Spatiocyte ($\Delta t=67\ \mathrm{\mu s},\ r=10\ \mathrm{nm},\ \mathrm{RT}=288\ \mathrm{s}$)','Mass Action']
speciesList = ['E','S','ES','P']
lines = ['-','--','-','--','-','--','-']
colors = ['b', 'b', 'r', 'r', 'g', 'g','k']
...
for f in range(len(fileNames)):
  ...
  for i in range(colSize):
    if (i == 0):
      plot(data[0], data[i+1], ls=lines[f], color=colors[f], label=legendTitles[f], linewidth=1.5)
    else:
      plot(data[0], data[i+1], ls=lines[f], color=colors[f], linewidth=1.5)
...
ax = gca()
leg = legend(loc=(0.02,0.225), labelspacing=0.3, handletextpad=0.2)
