...
imp.load_source('fastbd_out', 'fastbd/fastbd_out.py')
...
plot(Nv, fastbd_data,'yD', label=r'Fast BD ($V=3\ \mathrm{\mu m}^{3}$)')
loglog(X, 50*X, 'y--')
annotate(r'$T\propto N$', xy=(X[1], fastbd_data[2][0]),  xycoords='data', xytext=(-15, 0), textcoords='offset points', color='y', size=14)

plot(Nv, egfrd_dense_data,'bs', label=r'eGFRD ($V=3\ \mathrm{\mu m}^{3}$)')
loglog(X, 2e-2*X**(5.0/3.0), 'b--')
annotate(r'$T\propto N^{\mathsf{\frac{5}{3}}}$', xy=(X[9], egfrd_dense_data[7][0]),  xycoords='data', xytext=(-20, 15), textcoords='offset points', color='b', size=14)
...
annotate(r'$T\propto N$', xy=(X[7], smoldyn_data[6][0]),  xycoords='data', xytext=(-25, -23), textcoords='offset points', color='g', size=14)
